...
async def main(page: ft.Page):
    ...
    def verificar_actualizacion():
        ...
        # 1. ARREGLO LÍNEAS ROJAS: Validamos que 'config' sea realmente un diccionario
        if isinstance(config, dict) and config.get("version_actual") != VERSION_APP:
            
            # 2. ARREGLO DEL BOTÓN: Convertimos la función a asíncrona (async def)
            async def abrir_enlace_actualizacion(e):
                try:
                    # Usamos .get() de forma segura
                    url = str(config.get("enlace_descarga", "")).strip()
                    
                    if not url:
                        mostrar_error("Error: El enlace está vacío en la BD.")
                        return

                    mostrar_exito(f"Abriendo: {url}")
                    
                    # 3. EL TOQUE MÁGICO: Agregamos "await" para que realmente se ejecute la orden
                    await page.launch_url(url)
                    
                except Exception as ex:
                    mostrar_error(f"Fallo al abrir: {str(ex)}")
            ...
            dialogo_actualizacion = ft.AlertDialog(
                ...
                actions=[
                    ft.ElevatedButton(
                        ...
                        on_click=abrir_enlace_actualizacion, # Nuestra función asíncrona
                        scale=1.0,
                        animate_scale=ft.Animation(200, ft.AnimationCurve.EASE_OUT_CUBIC),
                        on_hover=animar_hover_boton
                    )
