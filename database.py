...
def actualizar_progreso_juego(id_jugador, niveles_completados, xp_total, racha, ultima_interaccion, dias_jugados_str):
    """Guarda el progreso de niveles, XP y la racha en la base de datos."""
    conexion = conectar_bd()
    if conexion:
        cursor = conexion.cursor()
        sql = """UPDATE progreso_jugadores 
                 SET niveles_completados = %s, xp_total = %s, racha = %s, ultima_interaccion = %s, dias_jugados = %s 
                 WHERE id = %s"""
        cursor.execute(sql, (niveles_completados, xp_total, racha, ultima_interaccion, dias_jugados_str, id_jugador))
        conexion.commit()
        cursor.close()
        conexion.close()
...
def obtener_configuracion_app():
    """Obtiene la configuración general de la app, como la versión actual."""
    conexion = conectar_bd()
    if conexion:
        cursor = conexion.cursor(dictionary=True)
        cursor.execute("SELECT * FROM configuracion_app WHERE id = 1")
